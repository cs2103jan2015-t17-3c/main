...
const int Parser::DEFAULT_INDEX = -1;
...
void Parser::processCommand(std::string input, std::string& commandType, std::string& userTask, std::vector<date>& vecStartDate, std::vector<date>& vecEndDate, std::vector<ptime>& vecStartTime, std::vector<ptime>& vecEndTime) {
	...
	ptime startTime;
	ptime endTime;
	//process the first part of the input string as per normal;
	processCommand(separateRecurringFront(input), commandType, userTask, startDate, endDate, startTime, endTime, dummyIndexReference);
	//return wrong commanType if the recurring task never indicate at least one start date
	...
	//pass to completeRecurring to complete two vectors of date and two vectors time
...
void Parser::processCommand(std::string input, std::string& commandType, std::string& userTask, date& startDate, date& endDate, ptime& startTime, ptime& endTime, int& indexReference) {
	...
	if(isRigid(input)) {
		...
		try{
			...
			//pass logic wrong commandType to indicate error
			commandType = EMPTY;
		}
	} else {
...
//The function can take in string and string of delimiters
//delimiters are used to separate the string and store the tokens in private scope
...
//only applicable to delete,complete and edit
...
//change the token's position to the actual position in the input string
...
//get frequency,recurrence,interval of a recurring task
//meanning the frquency of the recurring wg. every week
//         the special frequency eg. every two week
//           the number of times of recurring eg. 4
